...
const VENUS_PRESETS = ['#7C3AED', '#EC4899', '#06B6D4', '#10B981', '#F59E0B', '#EF4444'];

export const BrandingCustomization: React.FC = () => {
  const [previewColor, setPreviewColor]       = useState('#7C3AED');
  const [savedColor, setSavedColor]           = useState('#7C3AED');
  ...
  return (
    <div className="animate-fadeIn">
      <div className="mb-8">
        ...
        <p className="text-violet-400 text-sm">Customise colours, logo, and business hours.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="section-dark">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <input type="text" value={previewColor} onChange={(e) => setPreviewColor(e.target.value)}
                className="input-dark flex-1 font-mono" placeholder="#7C3AED" />
            </div>
            ...
            <p className="text-xs text-violet-500 mt-3">Used for buttons, highlights, and accent elements.</p>
          </div>
          ...
          <div className="section-dark">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button type="button" onClick={() => fileInputRef.current?.click()}
                className="btn-outline-venus text-xs py-2.5" style={{ color: '#A78BFA', borderColor: 'rgba(167,139,250,0.40)', background: 'transparent' }}>
            ...
            <p className="text-xs text-violet-500 mt-3">Recommended: 150 × 50 px, PNG or SVG.</p>
          </div>
            ...
            <div className="space-y-3">
              {(Object.entries(businessHours) as [DayKey, DayHours][]).map(([day, h]) => (
                <div key={day} className="flex items-center gap-3">
                  <span className="w-20 text-xs font-semibold text-violet-300 capitalize">{day}</span>
                  ...
                  <button
                    ...
                    style={{ background: h.isOpen ? '#7C3AED' : 'rgba(45,32,96,0.80)' }}
                  >
                  ...
                  <span className="text-xs text-violet-500 w-10">{h.isOpen ? 'Open' : 'Closed'}</span>
        ...
        <div>
          ...
          {/* Booking preview */}
          <div className="rounded-2xl overflow-hidden mb-5" style={{ border: '1px solid rgba(124,58,237,0.25)' }}>
            ...
            <div className="space-y-2">
              {(Object.entries(businessHours) as [DayKey, DayHours][]).map(([day, h]) => (
                <div key={day} className="flex justify-between text-xs">
                  <span className="capitalize text-violet-300 font-semibold">{day}</span>
                  <span className={h.isOpen ? 'text-white' : 'text-violet-500'}>
